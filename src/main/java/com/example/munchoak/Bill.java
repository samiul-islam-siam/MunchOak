...
public class Bill {
    ...
    public String generateReceipt(Map<Integer, FoodItems> foodMap) {
        ...
        sb.append("--------------------------------------------\n");
        sb.append(String.format("%-21s %15.2f\n", "Subtotal:", subtotal)); // Subtotal includes add-ons

        // Fixed values matching CartPage and CheckoutPage
        double delivery = 7.99;
        double tax = 7.00;
        double service = 1.50;
        double discount = payment.getDiscount(); // Use payment's discount
        double tip = payment.getTip(); // Use payment's tip

        // Match the order: Delivery, Tax, Service Fee, Tip, Discount
