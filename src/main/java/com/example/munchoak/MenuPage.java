...
public class MenuPage {
    ...
    private final User currentUser;  // multi-user ready
    private final Cart cart;

    public MenuPage(User user) {
        this.currentUser = user;
        this.cart = new Cart(String.valueOf(user.getId()));  // associate cart with current user
    }
    ...
    public Node getView() {
        ...
        HBox catButtons = new HBox(10, addCatBtn, renameCatBtn, deleteCatBtn);
        inputGrid.add(catButtons, 1, 5);
        ...
        payBtn.setStyle("-fx-background-color: #1abc9c; -fx-text-fill: white;");
        payBtn.setOnAction(e -> makePayment());

        HBox cartButtons = new HBox(15, viewCartBtn, checkoutBtn, payBtn);
    ...
    private void loadCategories() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) categoryBox.getItems().add(rs.getString("Category_Name"));
    ...
    private void renameCategory() {
        ...
        if (selected == null) { showAlert("Error", "Select a category first."); return; }
    ...
    private void deleteCategory() {
        String selected = categoryBox.getValue();
        if (selected == null) { showAlert("Error", "Select a category."); return; }
        ...
        confirm.showAndWait().ifPresent(res -> {
            if (res == ButtonType.YES) {
                try (Connection conn = DatabaseConnection.getConnection()) {
                    ...
                    loadFoodItems();
                } catch (SQLException ex) { showAlert("Error", "Delete failed."); }
            }
        });
    }
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                if (!categoryFlows.containsKey(category)) {
                    ...
                    catLabel.setStyle("-fx-font-size: 22px; -fx-font-weight: bold;");
                    ...
                    VBox section = new VBox(10, catLabel, sep, flow);
    ...
    private void makePayment() {
        ...
        for (FoodItems item : foodList) foodMap.put(item.getId(), item);

        double total = 0;

        // Calculate total
        for (Map.Entry<Integer, Integer> entry : cart.getBuyHistory().entrySet()) {
            FoodItems f = foodMap.get(entry.getKey());
            if (f != null) total += f.getPrice() * entry.getValue();
        }
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                stmt.setString(2, cart.getToken());
                stmt.setDouble(3, total);
                stmt.setString(4, cart.getTimestamp());
        ...
        successAlert.setContentText("Your payment was successful!");
    ...
    private VBox createFoodCard(FoodItems food) {
        ...
        try (InputStream is = getClass().getResourceAsStream(imagePath)) {
            ...
            else image = new Image("file:src/main/resources/images/" + food.getImagePath());
    ...
    private void addFoodItem() {
        if (categoryBox.getValue() == null || categoryBox.getValue().trim().isEmpty()) { showAlert("Error", "No category selected."); return; }
        if (nameField.getText().trim().isEmpty()) { showAlert("Error", "Food name cannot be empty."); return; }
        ...
        } catch (NumberFormatException e) { showAlert("Error", "Invalid numeric value."); return; }

        String img = selectedImageFile != null ? selectedImageFile.getName() : "";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            loadFoodItems();
            clearFields();
        } catch (SQLException e) { showAlert("Error", "Failed to add food item."); }
    }

    private void updateFoodItem() {
        if (currentEditingFood == null) return;
        String img = selectedImageFile != null ? selectedImageFile.getName() : currentEditingFood.getImagePath();
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            loadFoodItems();
            clearFields();
        } catch (SQLException e) { showAlert("Error", "Failed to update food item."); }
    }
    ...
    private void showCart() {
        ...
        TableView<CartItemView> table = new TableView<>();
        ObservableList<CartItemView> cartItems = FXCollections.observableArrayList();

        for (Map.Entry<Integer, Integer> entry : cart.getBuyHistory().entrySet()) {
            FoodItems f = foodMap.get(entry.getKey());
            if (f != null) cartItems.add(new CartItemView(f.getName(), entry.getValue(), f.getPrice()));
        }
        table.setItems(cartItems);

        TableColumn<CartItemView, String> nameCol = new TableColumn<>("Name");
        nameCol.setCellValueFactory(new javafx.scene.control.cell.PropertyValueFactory<>("name"));
        TableColumn<CartItemView, Integer> qtyCol = new TableColumn<>("Qty");
    ...
    public static class CartItemView {
        ...
        public CartItemView(String name, int qty, double price) {
            this.name = name;
            this.quantity = qty;
            this.price = price;
